...
**Done — confirmed by Roman on the production host: everything migrated and is working.**

1.0.1 installed, the legacy config migrated and the fleet came up. This is the first exercise of the shared `~/.pm2` path (05's rehearsal deliberately used an isolated `PM2_HOME` to avoid touching this machine's own daemon), and it went through without intervention — so the daemon-version mismatch that would have required a `pm2 kill` did not materialise.

Detailed per-step output was not captured; the operator's confirmation is the evidence of record.
