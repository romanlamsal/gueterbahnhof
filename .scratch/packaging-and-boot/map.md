...
## Destination — REACHED (2026-08-06)

1.0.1 shipped and is running on the production host: the legacy config migrated and the fleet came up, confirmed by the operator ([06](issues/06-upgrade-the-real-server.md)). Every ticket on this map is resolved; what remains sits in **Not yet specified** and **Out of scope** below, for a future effort.
...
- [Upgrade the real server and verify](issues/06-upgrade-the-real-server.md) — **destination reached**: 1.0.1 installed on the production host, legacy config migrated, fleet running. First exercise of the shared `~/.pm2` path, and it needed no intervention.
- [Implement 1.0.1 and prove it by clean install](issues/05-implement-and-prove-by-clean-install.md) — shipped in `28470eb`; a tarball installed outside the workspace migrated a legacy config and brought both apps online **with zero HTTP requests**, survived an env change across a restart, and left a foreign process and the daemon untouched on shutdown. Tarball: 1025 → 148 files.
